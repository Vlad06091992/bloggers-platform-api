...
describe('test for /posts', () => {
    ...
    it('post do not create because auth data is not valid', async () => {
        const data: PostCreateModel = {title: 'new title', blogId: '1', content: 'content', shortDescription: 'desc'};
        await postsTestManager.createPost(data, '', noValidAuthData, HTTP_STATUSES.UNAUTHORIZED_401, {})
    })

    it('post do not create because title is not valid and blog not found', async () => {
        const data: PostCreateModel = {title: '', blogId: '1', content: 'content', shortDescription: 'desc'};
        await postsTestManager.createPost(data, '', authData, HTTP_STATUSES.BAD_REQUEST_400, {
            "errorsMessages": [
                {
                    "field": "title",
    ...
    it('post do not create because content and shortDescription is not valid and blog is not found', async () => {
        const data: PostCreateModel = {title: 'new title', blogId: '1', content: '', shortDescription: ''};
        await postsTestManager.createPost(data, '', authData, HTTP_STATUSES.BAD_REQUEST_400, {
                "errorsMessages":
                    [
                        {
    ...
    it('post should be created', async () => {
        ...
        const result = await postsTestManager.createPost(data, blogName, authData, HTTP_STATUSES.CREATED_201, {})
        createdPost = result
    })
    ...
    it('post should not found by id', async () => {
        await postsTestManager.getPostById("2", 'some title', HTTP_STATUSES.NOT_FOUND_404)
    })
    ...
    it('should not updated new post with incorrect idEntity', async () => {
        await postsTestManager.updatePost('2', createdPost, authData, HTTP_STATUSES.NOT_FOUND_404, {})
    })

    it('should not updated new post with no valid authData', async () => {
        await postsTestManager.updatePost('2', createdPost, noValidAuthData, HTTP_STATUSES.UNAUTHORIZED_401, {})
    })

    it('post do not updated because title,content is not valid and blog is not found', async () => {
        await postsTestManager.updatePost(createdPost.id, {
            title: 'new title',
            blogId: '1',
            content: '',
            shortDescription: ''
        }, authData, HTTP_STATUSES.BAD_REQUEST_400, {
            "errorsMessages": [
                {
                    "field": "shortDescription",
                    "message": "The 'short description' field is required and must be no more than 500 characters."
    ...
    it('should be updated post with correct data', async () => {
        ...
        let updatedPost = await postsTestManager.getPostById(createdPost.id, createdPost.title, HTTP_STATUSES.OK_200)
    ...
    it('should not delete post with incorrect auth data', async () => {
        await postsTestManager.deletePost(createdPost.id, noValidAuthData, HTTP_STATUSES.UNAUTHORIZED_401)
    })

    it('should not delete no existing post', async () => {
        await postsTestManager.deletePost('2', authData, HTTP_STATUSES.NOT_FOUND_404)
    })
    //
    it('should delete existing post', async () => {
        await postsTestManager.deletePost(createdPost.id, authData, HTTP_STATUSES.NO_CONTENT_204)
    })
    it('should return status 200 and empty', async () => {
        await request(app)
            .get(Routes.posts)
